...
#define TP3680A_ID	0x88
#define TP3680B_ID	0x82
char chip_type=0x00;
struct fw_data
{
    u32 offset : 8;
...
#if defined (CONFIG_BOARD_TYPE_ZM1128CE)//add by xielei
...
#define X_POL
#elif defined(CONFIG_TP_3670_TAB785_FPCA79A29V02)
#include "gsl3670_tab785.h"
#define TP_SIZE_1920x1200
#define SWAP_XY
#define Y_POL
#elif defined(CONFIG_TP_3680_TAB106)
#include "gsl3680_tab106.h"
#include "gsl3680b_tab106.h"
#define TP_SIZE_1366X768
#define Y_POL
#elif defined(CONFIG_TP_3680_MGYCTP_10601)
#include "gsl3680_tab8010.h"
#define TP_SIZE_1280X800
#elif defined(CONFIG_TP_3680_MGYCTP_80600)
#include "gsl3680_tab8010_8d.h"
#define TP_SIZE_800X1280
#define X_POL
#define Y_POL
#else
...
//1536x2048
	...
	#define SCREEN_MAX_Y 		768
#elif defined(TP_SIZE_1280X800)	
	#define SCREEN_MAX_X 		1280
	#define SCREEN_MAX_Y 		800
#else
	#define SCREEN_MAX_X 		1536//2048
	#define SCREEN_MAX_Y 		2048//1536
	#define X_POL
	#define Y_POL
...
#endif 
#ifdef CONFIG_BOARD_TYPE_HW1089
extern int havn_pen_flags;
#endif
...
struct key_data gsl_key_data[MAX_KEY_NUM] = {
	{KEY_HOMEPAGE, 1070, 1090, 1550, 1570},
...
static int gslX680_init(void)
{
	...
	unsigned long home_flags;
	
	this_ts->irq = of_get_named_gpio_flags(np, "irq-gpio", 0, (enum of_gpio_flags *)&irq_flags);
	//#if !defined(CONFIG_BOARD_TYPE_ZM1128CE) 
	this_ts->rst = of_get_named_gpio_flags(np, "rst-gpio", 0, &rst_flags);
	printk("gsl********* reset pin =%d \n",this_ts->rst);

	this_ts->home = of_get_named_gpio_flags(np, "home-gpio", 0, &home_flags);
	printk("lsd gsl********* home pin =%d \n",this_ts->home);
   // gpio_set_value(this_ts->rst, 1);
	//#endif
	//this_ts->pwr_pin = of_get_named_gpio_flags(np, "power-gpio", 0, &pwr_flags);
	
	msleep(20);
	if(gpio_request(this_ts->rst,NULL) != 0){
		gpio_free(this_ts->rst);
		printk("gpio_request this_ts->rst error\n");
		return -EIO;
	}
...
static void judge_chip_type(struct i2c_client *client)
{
	...
	printk("leaf ggggg buf0 =%x %x %x  %x  \n",read_buf[0],read_buf[1],read_buf[2],read_buf[3]);
...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	#ifdef HAVE_TOUCH_KEY
		report_key(ts,x,y);
		//printk("lsd gsl this_ts->home=%d \n",gpio_get_value(this_ts->home));
		gpio_direction_output(this_ts->home, 1);
		//printk("lsd gsl this_ts->home=%d \n",gpio_get_value(this_ts->home));
		msleep(100);
		gpio_direction_output(this_ts->home, 0);
		//printk("lsd gsl this_ts->home=%d \n",gpio_get_value(this_ts->home));
	#endif
		return;
	}
...
#if 1 //  ifndef REPORT_DATA_ANDROID_4_0
/*	if(0 == touches){
...
//	print_info("----------------gsl_monitor_worker-----------------\n");	
	...
	//gsl_ts_read(gsl_client, 0x80, read_buf, 4);
...
static int gslX680_ts_init(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	//input_device->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) ;
	for (i = 0; i < MAX_KEY_NUM; i++)
	{
		input_device->evbit[i] =  BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) ;
//input_device->evbit[i] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS);
