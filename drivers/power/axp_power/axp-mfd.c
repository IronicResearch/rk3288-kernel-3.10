...
extern void rk_send_wakeup_key(void);//lsd add 2014-11-26
...
static void axp_mfd_irq_work(struct work_struct *work)
{
	...
	rk_send_wakeup_key();//lsd add 2014-11-26
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (ret)
		goto out_free_chip;
	printk("[axp-mfd] gpio_to_irq(4)=%d!\n",gpio_to_irq(4));
	printk("[axp-mfd] client->irq=%d!\n",client->irq);
	...
	enable_irq_wake(client->irq);//lsd add 2014-11-26
	...
	//lsd add enble dcdc3
	axp_write(&axp->dev, 0x3B, 0x10);
	//lsd add enble gpio0/1
	axp_write(&axp->dev, 0x90, 0x03);
	axp_write(&axp->dev, 0x92, 0x03);
	axp_write(&axp->dev, 0x91, 0x03);
	axp_write(&axp->dev, 0x93, 0x0B);
	return 0;
