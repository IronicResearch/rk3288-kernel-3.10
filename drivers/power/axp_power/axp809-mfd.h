...
static int axp_init_chip(struct axp_mfd_chip *chip)
{
	...
	if (err) {
	    printk("[AXP809-MFD] try to read chip id failed!\n");
		return err;
	}
	...
	if (err) {
	    printk("[AXP809-MFD] try to clear irq failed!\n");
		return err;
	}
	...
	if (err) {
	    printk("[AXP809-MFD] try 0x81 failed!\n");
		return err;
	}
	err =  __axp_read(chip->client, 0x81, &chip_id);
	if (err) {
	    //printk("[AXP809-MFD] read 0x81 0x%02x!\n",chip_id);
		return err;
	}
//	printk("[AXP809-MFD] read 0x81 0x%02x!\n",chip_id);
