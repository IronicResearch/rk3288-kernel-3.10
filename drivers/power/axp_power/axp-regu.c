...
#define AXP809_LDO(_id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)	\
	AXP_LDO(AXP, _id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)

#define AXP809_DCDC(_id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)	\
	AXP_DCDC(AXP, _id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)

#define AXP809_SW(_id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)	\
	AXP_SW(AXP, _id, min, max, step1, vreg, shift, nbits, ereg, ebit, switch_vol, step2, new_level)


static struct axp_regulator_info axp_regulator_info[] = {
	AXP809_LDO(1,	3000,	3000,	0,	LDO1,	0,	0,	LDO1EN,	0, 0, 0, 0),//ldo1 for rtc
	AXP809_LDO(2,	700,	3300,	100,	LDO2,	0,	5,	LDO2EN,	6, 0, 0, 0),//ldo2 for aldo1
	AXP809_LDO(3,	700,	3300,	100,	LDO3,	0,	5,	LDO3EN,	7, 0, 0, 0),//ldo3 for aldo2
	AXP809_LDO(4,	700,	3300,	100,	LDO4,	0,	5,	LDO4EN,	5, 0, 0, 0),//ldo3 for aldo3
	AXP809_LDO(5,	700,	4200,	100,	LDO5,	0,	5,	LDO5EN,	3, 3400, 200, 0),//ldo5 for dldo1
	AXP809_LDO(6,	700,	3300,	100,	LDO6,	0,	5,	LDO6EN,	4, 0, 0, 0),//ldo6 for dldo2
	AXP809_LDO(9,	700,	3300,	100,	LDO9,	0,	5,	LDO9EN,	0, 0, 0, 0),//ldo9 for eldo1
	AXP809_LDO(10,	700,	3300,	100,	LDO10,	0,	5,	LDO10EN,1, 0, 0, 0),//ldo10 for eldo2
	AXP809_LDO(11,	700,	3300,	100,	LDO11,	0,	5,	LDO11EN,2, 0, 0, 0),//ldo11 for eldo3
	AXP809_LDO(12,	700,	3300,	100,	LDO12,	0,	3,	LDO12EN,0, 0, 0, 0),//ldo12 for dc5ldo
	AXP809_DCDC(1,	1600,	3400,	100,	DCDC1,	0,	5,	DCDC1EN,1, 0, 0, 0),//buck1 for io
	AXP809_DCDC(2,	600,	1540,	20,	DCDC2,	0,	6,	DCDC2EN,2, 0, 0, 0),//buck2 for cpu
	AXP809_DCDC(3,	600,	1860,	20,	DCDC3,	0,	6,	DCDC3EN,3, 0, 0, 0),//buck3 for gpu
	AXP809_DCDC(4,	600,	2600,	20,	DCDC4,	0,	6,	DCDC4EN,4, 1540, 100, 1800),//buck4 for core
	AXP809_DCDC(5,	1000,	2550,	50,	DCDC5,	0,	5,	DCDC5EN,5, 0, 0, 0),//buck5 for ddr
	AXP809_LDO(IO0,	700,	3300,	100,	LDOIO0,	0,	5,	LDOIO0EN,0, 0, 0, 0),//ldoio0
	AXP809_LDO(IO1,	700,	3300,	100,	LDOIO1,	0,	5,	LDOIO1EN,0, 0, 0, 0),//ldoio1
	AXP809_SW(0,	700,	3300,	100,	SW0,	0,	0,	SW0EN,6, 0, 0, 0),//SW0
};
...
static int axp_regulator_probe(struct platform_device *pdev)
{
	...
	if (ri == NULL) {
		dev_err(&pdev->dev, "invalid regulator ID specified\n");
			printk("[AXP809-MFD] ri == NULL\n");
		return -EINVAL;
	}
	printk("[AXP809-MFD] ri->desc.id=%d\n!",ri->desc.id);
	if (ri->desc.id == AXP_ID_LDO1 || ri->desc.id == AXP_ID_LDO2 \
		|| ri->desc.id == AXP_ID_LDO3 || ri->desc.id == AXP_ID_LDO4 \
		|| ri->desc.id == AXP_ID_LDO5 || ri->desc.id == AXP_ID_LDO6 \
		|| ri->desc.id == AXP_ID_LDO7 || ri->desc.id == AXP_ID_LDO8 \
		|| ri->desc.id == AXP_ID_LDO9 || ri->desc.id == AXP_ID_LDO10 \
		|| ri->desc.id == AXP_ID_LDO11 || ri->desc.id == AXP_ID_LDO12 \
		|| ri->desc.id == AXP_ID_DCDC1 || ri->desc.id == AXP_ID_DCDC2 \
		|| ri->desc.id == AXP_ID_DCDC3 || ri->desc.id == AXP_ID_DCDC4 \
		|| ri->desc.id == AXP_ID_DCDC5 || ri->desc.id == AXP_ID_SW0)
		ri->desc.ops = &axp_ops;
	if (ri->desc.id == AXP_ID_LDOIO0|| ri->desc.id == AXP_ID_LDOIO1 )
	...
	axp_write(axp_dev, 0x10, 0xef);
	ret = axp_read(axp_dev, 0x10, &val);

	if(ri->desc.id == AXP_ID_DCDC1 ||ri->desc.id == AXP_ID_DCDC2 \
		|| ri->desc.id == AXP_ID_DCDC3 ||ri->desc.id == AXP_ID_DCDC4 \
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Criss");
MODULE_DESCRIPTION("Regulator Driver for X-powers AXP809 PMIC");
