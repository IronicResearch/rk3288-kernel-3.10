...
#endif
#include "axp_suspend.h"
struct axp_device  axp_fb;
...
#if defined(CONFIG_BOARD_TYPE_TAB9689)||defined(CONFIG_BOARD_TYPE_TAB106)
#define lsd_change_wake 1
#else
#define lsd_change_wake 0
#endif
#if lsd_change_wake
static struct wake_lock change_wake_lock;
#endif
void lsd_axp_clr_power()
{
	printk("lsd axp_power_off\n");
	axp_set_bits(axp_charger->master, AXP_OFF_CTL, 0x80);
}
...
#endif
static int axp_suspend_cb(void)
{
	printk("axp axp_suspend \n");
	int tmp;
...
#if lsd_change_wake
	uint8_t	lsd_charge_status;
	axp_read(axp_charger->master, 0x00,&lsd_charge_status);
	//printk("lsd charge 0x00h status: %d\n",lsd_charge_status);
	if(0!=((lsd_charge_status & 0x80)||(lsd_charge_status & 0x20)))
		wake_lock(&change_wake_lock);
	else
		wake_unlock(&change_wake_lock);
#endif
	return 0;

}
static int axp_resume_cb(void)
{
	printk("axp resume \n");
	int tmp;
...
//probe 时初始化RDC，使其提前计算正确的OCV，然后在此处启动计量系统
  ...
  /* set usb cur-vol limit*/
	...
	/*给局部变量赋值*/
	axp_charger = charger;
	axp_fb.axp_suspend = &axp_suspend_cb;
	axp_fb.axp_resume = &axp_resume_cb;
	axp_register_fb(&axp_fb);
#ifdef CONFIG_HAS_EARLYSUSPEND
...
#endif
#if lsd_change_wake
wake_lock_init(&change_wake_lock, WAKE_LOCK_SUSPEND, "change_lock");
#endif
	/* 调试接口注册 */
